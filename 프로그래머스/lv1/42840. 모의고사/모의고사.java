...
class Solution {
    public int[] solution(int[] answers) {
        ...
        for(int i=0; i<answers.length; i++) {                                   // 수포자들의 점수 계산
            if(answers[i] == first[i%5]) score[0]++;
            if(answers[i] == second[i%8]) score[1]++;
            if(answers[i] == third[i%10]) score[2]++;
        }
        
        int max = Math.max(score[0], Math.max(score[1], score[2]));             // 최대 점수 구하기
        
        List<Integer> answ = new ArrayList<Integer>();
        for(int i=0; i<score.length; i++) if(max == score[i]) answ.add(i+1);    // 최대 점수를 가진 수포자 리스트 생성
